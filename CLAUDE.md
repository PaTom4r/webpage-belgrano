...
- **Hero — Living Threads Canvas 2D** (`src/components/particles/living-pillar/`): 8 bandas horizontales (3-3-2 back/mid/front) con amp/freq/speed/fase/grosor propios. Partículas paramétricas (t ∈ [0,1] sobre la curva de cada banda) **conectadas por líneas finas translúcidas** que leen como fibras musculares / corrientes de datos. 1440 partículas desktop / 700 mobile (densidad menor compensada por el lineado). Render en 3 pasadas back-to-front: lines (source-over alpha bajo) + glow (lighter, solo bandas depth ≥ 1, sprite radial cacheado por bucket de radio) + core (arc fill modulado por depth alpha/size).
...
- **Skip de segments largos**: `MAX_SEGMENT_PX=25` evita que las fibras dibujen diagonales feas cuando el cursor empuja una partícula fuera de banda.
- **Layout hero**: lg+ canvas `absolute inset-0` z-0; dos overlays z-10 (Tailwind `bg-gradient-to-l from-black via-black/70 to-transparent` + radial inline `at 75% 50%` rgba 0.75→0.35→transparent, desktop only); contenido z-20 a `lg:pl-[51vw]` con `max-w-[620px]`. Mobile: canvas arriba 55-60vh con `opacity-60`, texto abajo, sin radial.
- **Headline**: `BELGRANO` arriba / `GROUP` abajo, **dos líneas siempre** vía `<span className="block">` (sin `lg:whitespace-nowrap`). Tamaños fluidos: text-5xl → text-6xl → text-[5rem] → text-[6rem] → text-[7rem]. Subtítulo `max-w-[560px]`.
