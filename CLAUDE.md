...
- **Branch activa**: `feature/hero-threads-refactor`, pusheada desde `feature/hero-particle-entity`. El refactor evoluciona el wave field a fibras conectadas; ambos branches viajan juntos al merge.
...
- **Layout hero**: lg+ canvas `absolute inset-0` z-0; dos overlays z-10 (Tailwind `bg-gradient-to-l from-black via-black/70 to-transparent` + radial inline `at 75% 50%` rgba 0.75→0.35→transparent, desktop only); contenido z-20 a `lg:pl-[40vw]` (xl:42vw) con `max-w-[680px]`. Mobile: canvas arriba 55-60vh con `opacity-60`, texto abajo, sin radial.
...
- **VerticalesReveal scroll-driven** (`src/components/sections/home/verticales-reveal.tsx`, traída por cherry-pick desde el experimento revertido en main): reemplaza al accordion HeroMockups Zone 2. Sección sticky 700vh con video Veo 3.1 (oficina Santiago + hologramas, `public/home/intelligence.mp4` 2.3MB H.264) como fondo full, mask-gradient izquierdo + tint accent interpolado por vertical. Rail de texto sticky a la izquierda ("Tres verticales, un mismo equipo" + 01/02/03 + progress bar gradient). Panels Intelligence → Media → Brand cross-fade scroll-driven. Demo Intelligence: orquestador de cobranza en 3 pasos (Entrante / Orquestando / Resuelto) con chat WhatsApp + grid 4 sistemas + métricas. Custom hook `useElementProgress` (rAF + MotionValue) reemplaza framer-motion `useScroll` por glitches bajo Next 16 + Turbopack + React 19. Mobile (<md): stacks a 3 cards full-width sin sticky.
...
- **Validar visualmente `feature/hero-threads-refactor`** en local (`npm run dev`) y luego en Vercel preview: zoom 200%, sweep cursor, comparar contra el wave field. Si Pato OK → mergear ambas branches (threads-refactor → hero-particle-entity → main, o cerrar el PR original y abrir uno nuevo desde threads-refactor).
- **Color por vertical en partículas (Phase 2 diferida)**: hacer que las bandas cambien de tinte (Intelligence teal `#20808D` / Media sky `#0EA5E9` / Brand orange `#F97316`) según scroll a lo largo del hero o de la home. Diferido hasta que el motor de fibras esté validado en preview.
